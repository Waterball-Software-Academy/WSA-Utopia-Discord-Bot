...
fun GuessNum1A2BListerner(wsa: WsaDiscordProperties, jda: JDA) = listener {
    ...
    on<MessageReceivedEvent> {
        ...
        if (channel.id != guessNumberChannelId || message.author.isBot) {
            return@on
        }
        if (message.contentDisplay.length != 4 || !message.contentDisplay.matches("\\d+(\\.\\d+)?".toRegex())) {
            return@on
        }
